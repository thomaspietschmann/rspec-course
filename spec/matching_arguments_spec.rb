RSpec.describe 'matching arguments' do
  it 'can return different values depending on the argument' do
    ...
    allow(three_element_array).to receive(:first).with(1).and_return([1]) # 1 is the argument given
    allow(three_element_array).to receive(:first).with(2).and_return([1, 2]) # 2 is the argument given
    allow(three_element_array).to receive(:first).with(be >= 3).and_return([1, 2, 3]) # 3 is the argument given
